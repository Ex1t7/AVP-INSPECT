    ...
    for app_id, manifest_info in manifest_data.items():
        ...
        if manifest_info['ns_privacy_tracking'] == False:
            ...
            if tracking_domains:
                ...
                violation = {
                    ...
                    'tracking_domains_found': ', '.join(unique_tracking_domains[:5])  
                }
        ...
        for _, row in app_flows.iterrows():
            if is_tracking_domain(row['destination']):
                ...
                domain_simple = row['destination'].split('/')[0].split(':')[0]  
        ...
        if missing_domains:
            violation = {
                ...
                'missing_domains': ', '.join(list(missing_domains)[:10])  
            }
            violations.append(violation)
        ...
        if len(manifest_info['ns_privacy_collected_data_types']) > 0:
            ...
            if missing_data_types:
                violation = {
                    ...
                    'source': 'manifest',
                    'violation_type': 'neglect_disclosure',  
                    'collected_type': ', '.join(list(missing_data_types)[:5]),  
                    'expected_apple_types': None,
                    ...
                    'missing_data_types': ', '.join(list(missing_data_types)[:10])  
                }
                violations.append(violation)
